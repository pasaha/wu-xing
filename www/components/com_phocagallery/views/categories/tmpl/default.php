...
if ($this->tmpl['phocagallerywidth'] != '') {
	...
	echo '<div id="phocagallery" class="phocagallery__sectins">';
}
...
if ($this->tmpl['displayimagecategories'] == 1) {	
	for ($i = 0; $i < $countCategories; $i++) {
		if ( $columns == 1 ) {
			echo '<table class="preview__items">';
		} else {
			$float = 0;
			...
			if ($float == 1) {		
				echo '<div class="preview__items__wrapper"><table class="preview__items">';
			}
		}

		echo '<tr>';		
		echo '<td class="preview__image"><a href="'.$this->categories[$i]->link.'">';
		...
		echo '</a></td>';
		echo '<td class="preview__dsc"><a href="'.$this->categories[$i]->link.'" class="category'.$this->params->get( 'pageclass_sfx' ).'">'.$this->categories[$i]->title.'</a>&nbsp;';
