	...
	function extract()
	{
		...
		switch($extension)
		{
			case 'zip': // A ZIP archive
				jpimport('misc.unzip');
				$unarchiver = new CSimpleUnzip($archiveFilename);
				$ret = $unarchiver->ExtractFile($offset, $addPrefix);
				if($ret === false)
				{
					// An error occured
				...
				$offset = $ret['zipoffset'];
				$done = $ret['done'];
				break;
				
			case 'jpa': // A JPA archive
				jpimport('misc.unjpa');
				$unarchiver = new CUnJPA($archiveFilename);
				$ret = $unarchiver->ExtractFile($offset, $addPrefix);
