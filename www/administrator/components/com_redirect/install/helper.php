<?php
/**
 * @version		$Id: helper.php 343 2010-10-18 01:41:23Z eddieajau $
 * @package		NewLifeInIT
...
abstract class PackageInstallerHelper
{
	 ...
	 * @since	1.0
	 */
	public static function displayInstalled(&$modules, &$plugins, $title)
	{
?>
<?php echo $title;?>
...
<table class="adminlist">
	...
	<tbody>
		<tr class="row0">
			<td class="key" colspan="2"><?php echo JText::sprintf('J_INSTALL_COMPONENT', 'Jentla Audit'); ?></td>
...
<?php
	 ...
	 * @since	1.0
	 */
	public static function displayUninstalled(&$modules, &$plugins, $title)
	{
?>
		...
		if (is_a($modules, 'JSimpleXMLElement') && count($modules->children())) {
			foreach ($modules->children() as $module)
			{
				...
				if ($mtitle && $mposition) {
					...
					if (!$row->store()) {
						// Install failed, roll back changes
						$installer->parent->abort(JText::_('J_INSTALL_MODULE').' '.JText::_('J_INSTALL_INSTALL').': '.$db->stderr(true));
						return false;
					}
				}
